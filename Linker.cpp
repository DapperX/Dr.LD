...
void Linker::operator()(const char *filename)
{
    ...
    for (size_t i = 0; i < num_files; ++i) {
        ...
        for (auto it = secs.begin(), end = secs.end(); it != end; ++it) {
            ...
            for (auto sym_it = symtab.begin(), end = symtab.end(); sym_it != end; ++sym_it) {
                ...
                // std::cerr << "sym name \"" << (const u8*)&*(strtab.begin() + sym.st_name) << "\"" << std::endl;
                ...
                symbol_table.put((const u8*)&*(strtab.begin() + old_name), sym.st_size,
